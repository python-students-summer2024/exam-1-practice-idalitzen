...
def get_guess(max_value):
    guess_human=(input(f"Guess a number between 1 and {max_value} :"))
    if not guess_human.isdigit():
        return -1
    guess_human=int(guess_human)
    random_int=get_random_int(1, max_value)
    if guess_human < 1 or guess_human>max_value:
        return -1
    if guess_human == random_int:
        return True
    else:
        return False
